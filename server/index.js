import {getHierarchy,saveHierarchy} from './hierarchy.js';
import {renderTree,chartControls} from '../public/hierarchy-view.js';
...
async function handle(request,env){
 ...
 if(path.startsWith('/api/admin/')){
  ...
  if(path==='/api/admin/hierarchy'){if(user.role==='staff')fail('Only the primary administrator can edit parish leadership.',403);if(request.method==='GET')return json(await getHierarchy(env,true));if(request.method==='PUT')return json(await saveHierarchy(env,user,await readJson(request)));fail('Method not allowed.',405);}
 ...
 if(path.startsWith('/api/public')){
  ...
  if(path==='/api/public/hierarchy')return json(await getHierarchy(env));
 ...
 if(path==='/hierarchy'){const h=await getHierarchy(env);return htmlPage('Parish leadership',`<div class="eyebrow">TOGETHER IN SERVICE</div><h1>Parish leadership</h1><p>Our ministries and the people who serve under the leadership of the Parish Priest.</p><p><a class="text-link" href="/people">Parish people & service history →</a></p><section id="public-hierarchy">${chartControls()}<div class="hierarchy-viewport" tabindex="0" aria-label="Scrollable parish leadership chart">${renderTree(h.nodes)}</div></section>`);}
