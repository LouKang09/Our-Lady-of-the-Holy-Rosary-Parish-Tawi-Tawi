...
async function saveSettings(request,env,user){if(request.method==='GET')return json(await settings(env));if(request.method!=='PUT')fail('Method not allowed.',405);const input=await readJson(request),previous=await settings(env);if(input.version!==previous.version)fail('Settings have changed. Reload before saving.',409);const value=validateSettings(input);await verifyImages(env,[...new Set([value.heroImage,value.aboutImage,...(value.heroImages||[])].filter(Boolean))]);const now=new Date().toISOString(),mutation=id('change_');const sql=previous.version?db(env).prepare('UPDATE settings SET data=?,version=version+1,updated_at=?,mutation=? WHERE id=? AND version=?').bind(JSON.stringify(value),now,mutation,'parish',previous.version):db(env).prepare('INSERT OR IGNORE INTO settings (id,data,version,updated_at,mutation) VALUES (?,?,1,?,?)').bind('parish',JSON.stringify(value),now,mutation);
