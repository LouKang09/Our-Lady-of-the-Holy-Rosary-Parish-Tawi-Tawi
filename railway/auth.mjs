...
export function createAuth(db,config){
 ...
 function user(request){const token=cookieToken(request);if(!/^[a-f0-9]{64}$/.test(token))return null;return sql.prepare('SELECT a.id,a.email FROM sessions s JOIN accounts a ON a.id=s.account_id WHERE s.token_hash=? AND s.expires_at>?').get(hash(token),Date.now())||null;}
 ...
 async function handle(request,ip='unknown'){
  ...
  try{
   if(route==='/api/auth/setup'){
    ...
    sql.exec('BEGIN IMMEDIATE');try{if(sql.prepare('SELECT id FROM accounts LIMIT 1').get()){sql.exec('ROLLBACK');return json({error:'Setup has already been completed. Please sign in.'},409);}sql.prepare('INSERT INTO accounts (id,email,password_hash,created_at) VALUES (?,?,?,?)').run(account.id,account.email,encoded,new Date().toISOString());sql.exec('COMMIT');}catch(error){sql.exec('ROLLBACK');throw error;}
   ...
   if(route==='/api/auth/password'){
    ...
    const encoded=await passwordHash(input.newPassword);sql.exec('BEGIN IMMEDIATE');try{sql.prepare('UPDATE accounts SET password_hash=? WHERE id=?').run(encoded,account.id);sql.prepare('DELETE FROM sessions WHERE account_id=?').run(account.id);sql.exec('COMMIT');}catch(error){sql.exec('ROLLBACK');throw error;}
   ...
   if(!account||!valid)return json({error:'The email or password is incorrect.'},401);
